...
<body>
    <?php 
        ...
        $stmt = $conn ->prepare("SELECT bed_no FROM bed_manage WHERE name='".$_SESSION['patient']."'"); //先將本病患原床位取出
        $stmt->execute();
        $Bf_bed_no = $stmt->fetchAll()[0][0];

        $stmt = $conn ->prepare("SELECT name FROM bed_manage WHERE bed_no='".$updata[2]."'"); //將第二位病患名子取出
        $stmt->execute();
        $Af_bed_name = $stmt->fetchAll()[0][0];

        if($Af_bed_name == NULL){ //若換過去的床位為空床，則只更新bed_manage的資料
            ...
            $stmt->execute();
            $data_change_text = "床號".$Bf_bed_no."(原病患".$_SESSION['patient'].")更換至床號".$Af_bed_no."<br>"; //將異動資訊更新，並更新至資料庫
            $data_change_text .= $_SESSION['data_change_text'];
            $_SESSION['data_change_text'] = $data_change_text;
            $stmt = $conn ->prepare("UPDATE data_change_text SET text = '".$data_change_text."' WHERE id = 1");
            $stmt->execute();
        }
        else{
            ...
            $stmt->execute();
            $Af_bed_no = $stmt->fetchAll()[0][0];
            $data_change_text = "床號".$Bf_bed_no."(原病患".$_SESSION['patient'].")更換至床號".$Af_bed_no."，與病患".$Af_bed_name."交換<br>"; //將異動資訊更新，並更新至資料庫
            $data_change_text .= $_SESSION['data_change_text'];
            $_SESSION['data_change_text'] = $data_change_text;
            $stmt = $conn ->prepare("UPDATE data_change_text SET text = '".$data_change_text."' WHERE id = 1");
            $stmt->execute();
        ...
        try {
            ...
            for($i = 0 ; $i < count($data) ; $i++){ //將要更新資料更新至資料庫
                $stmt = $conn ->prepare("UPDATE patient_admission_info SET ".$data[$i]."='".$updata[$i]."' WHERE name = '".$_SESSION['patient']."'");
                $stmt->execute();
            }
            $stmt = $conn ->prepare("UPDATE bed_manage SET bed_no = '".$updata[2]."' WHERE name = '".$_SESSION['patient']."'");
            $stmt->execute();
